...
class CompanyAddressModal extends ModalComponent
{
    #[Locked]
    public $id;
    ...
    public function mount($companyId, $id = null, $isStart)
    {
        ...
        if ($id) {
            $this->id = $id;
            $this->address = CompanyAddressChange::find($id);
        ...
        if (!$this->isStart) {
            $this->changeNature = 'Change of registered addresses';
        }
    }

    public function render()
    {
        return view('livewire.tenant.components.corporate-info.company-address-modal');
    }
    ...
    public function submit()
    {
        $this->validate();
        if (!$this->id) {
            $this->address = CompanyAddressChange::create(
                $this->formData($this->all(), $this->companyId)
            );

            $this->closeModalWithEvents([
                CompanyAddress::class => 'successCreated'
            ]);
        } else {
            $this->address->update(
                $this->formData($this->all(), $this->companyId)
            ...
            $this->closeModalWithEvents([
                CompanyAddress::class => 'successUpdated'
            ]);
        }
    }
