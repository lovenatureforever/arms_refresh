...
class SigAccPolicies extends Component
{
    #[Locked]
    public $id;
    ...
    {
        $this->loadSigAccPolicies();
        return view('livewire.tenant.pages.report-config.ntfs.sig-acc-policies', []);
    }
    ...
    {
        $this->items = NtfsConfigItem::where('company_id', $this->id)->where('section', 'sap')->orderBy('order')->get();
    }

    private function initializeSigAccPolicies()
    {
        $defaultItems = DefaultNtfsConfigItem::where('section', 'sap')->get();
