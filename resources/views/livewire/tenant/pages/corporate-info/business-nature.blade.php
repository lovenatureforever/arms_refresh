        ...
        <div class="p-6">
            <div class="grid grid-cols-1 gap-5" data-fc-type="tab">
                <nav class="flex-wrap rounded md:flex-col" role="tablist" aria-label="Tabs">
                    ...
                    <a class="btn border border-gray-100 mr-2 mb-2 active bg-transparent" role="tab" href="{{ route('corporate.companyname', ['id' => $id]) }}">
                        Company Name
                    </a>
                    <a class="btn border border-gray-100 mr-2 mb-2 bg-primary text-white" role="tab" href="{{ route('corporate.businessnature', ['id' => $id]) }}">
                        Nature of Business
                    </a>
                        ...
                        <div>
                            ...
                            <div class="relative overflow-x-auto border rounded-lg">
                                <div class="grid grid-cols-[repeat(2,_1fr)_0.4fr] text-xs font-bold text-center text-gray-700 uppercase border-b rounded-lg rounded-b-none bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                    ...
                                    <div class="flex items-center justify-center p-4 border-r ">Paragraph No</div>
                                    <div class="flex items-center justify-center p-4 border-r ">Principal Activities</div>
                                    <div class="flex items-center justify-center p-4">Action</div>
                                </div>
                                <div class="grid grid-cols-[repeat(2,_1fr)_0.4fr]">
                                    <div class="p-4 border-b border-r">Paragraph 1</div>
                                    <div class="p-4 border-b border-r">{{ $businessAtStart->paragraph1 }}</div>
                                    <div class="flex items-center justify-center row-span-2 p-4 border-b">
                                        <button class="mr-1 text-white btn btn-sm bg-info"
                                            wire:click="$dispatch('openModal', {component: 'tenant.components.corporate-info.business-nature-modal', arguments: { companyId : {{ $id }}, id: {{ $businessAtStart->id }}, isStart: true }})"
                                            ><i class="text-xl mgc_edit_line"></i>
                                        </button>
                                    </div>

                                    <div class="p-4 border-r">Paragraph 2</div>
                                    <div class="p-4 border-r">{{ $businessAtStart->paragraph2 }}</div>
                                </div>
                            </div>

                            <div class="flex flex-row justify-between mt-8">
                                <h1 class="mb-4 text-xl">Changes during the year</h1>
                                <div>
                                    <button class="text-white btn btn-sm bg-info" type="button" wire:click="$dispatch('openModal', {component: 'tenant.components.corporate-info.business-nature-modal', arguments: { companyId : {{ $id }}, id: null, isStart: false }})">Create</button>
                                </div>
                            </div>

                            @foreach ($businessChangesCurrentYear as $business)
                            <div class="relative mb-4 overflow-x-auto border rounded-lg">
                                <div class="grid grid-cols-[repeat(3,_1fr)_0.4fr] text-xs font-bold text-center text-gray-700 uppercase border-b rounded-lg rounded-b-none bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                    <div class="flex items-center justify-center p-4 border-r ">Paragraph No</div>
                                    <div class="flex items-center justify-center p-4 border-r">Principal Activities</div>
                                    <div class="flex items-center justify-center p-4 border-r">Effective Date</div>
                                    <div class="flex items-center justify-center p-4">Actions</div>
                                </div>
                                <div class="grid grid-cols-[repeat(3,_1fr)_0.4fr]">
                                    <div class="p-4 border-b border-r">Paragraph 1</div>
                                    <div class="p-4 border-b border-r">{{ $business->paragraph1 }}</div>
                                    <div class="flex items-center justify-center row-span-2 p-4 border-r">{{ $business->effective_date->format('Y-m-d') }}</div>
                                    <div class="flex items-center justify-center row-span-2 p-4 border-b">
                                        <button class="mr-1 text-white btn btn-sm bg-info"
                                            wire:click="$dispatch('openModal', {component: 'tenant.components.corporate-info.business-nature-modal', arguments: { companyId : {{ $id }}, id: {{ $business->id }}, isStart: false }})"><i
                                                class="text-xl mgc_edit_line"></i></button>
                                        <button class="text-white btn btn-sm bg-warning" type="button"
                                            wire:click="deleteBusinessNature({{ $business->id }})"
                                            wire:confirm="Are you sure you want to delete this?"><i class="text-xl mgc_delete_line"></i></button>
                                    </div>
                                    <div class="p-4 border-r">Paragraph 2</div>
                                    <div class="p-4 border-r">{{ $business->paragraph2 }}</div>
                                </div>
                            </div>
                            @endforeach
                                ...
                                <div>
                                    <div class="relative overflow-x-auto border rounded-lg">
                                        <div class="grid grid-cols-1 text-xs font-bold text-center text-gray-700 uppercase border-b rounded-lg rounded-b-none bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                            <div class="flex items-center justify-center p-4 ">Principal Activities Display on Report</div>
                                        </div>
                                        <div class="grid grid-cols-1">
                                            <div class="p-4">
                                                <p>{{ $businessAtLast->paragraph1 }}</p>
                                                <p>{{ $businessAtLast->paragraph2 }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
